...
class BattleshipBoard:
    def __init__(self) -> None:
        ...
        self.shots: dict[tuple[int, int], str] = {}  # pos -> "HIT"|"MISS"
...
def _tests_bs_engine() -> None:
    rng = random.Random(7)
    b = BattleshipBoard()
    b.place_ship("A", (0, 0), 3, "E")
    try:
        b.place_ship("B", (2, 0), 2, "S")
        assert False
    except RuntimeError:
        pass

    assert b.shoot((0, 0)) == "HIT"
    assert b.shoot((5, 5)) == "MISS"
    try:
        b.shoot((5, 5))
        assert False
    except RuntimeError:
    ...
    assert b.sunk_ships() == []
    b.shoot((1, 0)); b.shoot((2, 0))
    assert b.sunk_ships() == ["A"]

    b2 = BattleshipBoard()
    b2.place_random("R", 3, rng)
    assert "R" in b2.ships
